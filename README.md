...
Big O исчисляется в количестве операций n.
...
Реализация: <br/>
Создать два ключа, которые ссылаются – один на последний элемент в связном списке, другой – на предыдущий элемент в связном списке. <br/> После чего описываются разные варианты: когда нет ни единого элемента в связном списке, когда один элемент и когда два и больше элемента. <br/> <br/>
Big O(1): <br/>
...
Big O(n): <br/>
Поскольку используется метод Get и после чего вставляется новый элемент, то метод Insert использует цикл, для того, чтобы пробежаться по тому количеству элементу, скольким равен сам индекс. <br/> <br/>
...
Big O(n): <br/>
Поскольку используется метод Get и после чего привязывается заново существующий элемент, то метод Remove использует цикл, для того, чтобы пробежаться по тому количеству элементу, скольким равен сам индекс. <br/> <br/>

#### Reverse 
Перевернуть весь связной список. <br/> <br/>
Реализация: <br/>
У связного списка есть два основополагающих ключа один из которых обозначает начало связного списку (head), а другой – конец связного списка (tail). Ему присваиваются противоположные элементы, после чего все элементы перезаписывают ссылки в обратную сторону. <br/> <br/>
...
- next и prev – ключи, которые являются ссылками на предыдущий и следующий элемент связного списка. <br/>
...
Pop удаляет с конца элемент связного списка. <br/> <br/>
Реализация: <br/>
Создать два ключа, которые ссылаются – один на последний элемент в двойном связном списке, другой – на предыдущий элемент в связном списке. Перенести tail на предыдущий элемент, ссылку на следующий элемент (next) у этого, предыдущего элемента присвоить Null. <br/> <br/>
Big O(1): <br/>
...
Big O(1): <br/>
При добавлении в начало, происходит сдвиг на один элемент и одновременно с этим ссылка на следующий элемент (next) в новом элементе привязывается на первый элемент и одновременно с этим ссылке на предыдущий элемент (prev) присваивается Null, это занимает одну операцию. <br/> <br/>
...
Добавляет один элемент в средину связного списка. <br/> <br/>
Реализация: <br/>
Использовать ранее созданный метод Get, чтобы получить индекс предыдущего элемента. После чего перезаписать ссылку этого элемента на тот, который добавляется методом Insert, после чего связующей ссылке у самого элемента вставленного с помощью метода Insert присвоить элемент, ранее присвоенный предыдущему элементу полученного с помощью Get с помощью ключей prev, а следующий с помощью next. <br/> <br/>
...
Big O(n): <br/>
Поскольку используется метод Get и после чего привязывается наново существующий элемент, то метод Remove использует цикл, для того, чтобы пробежаться по тому количеству элементу, скольким равен сам индекс. <br/> <br/>
...
Стек образовывается за счёт двух классов: первый класс это сам элемент стека, а второй класс сам стек, который реализовывает методы по управлению элементами. <br/> <br/>

### Методы

Методами стека являются: <br/>
- добавление и удаление элементов в стеке. <br/>
- проверка на пустоту стека. <br/>
- получение значения верхнего элемента в стеке. <br/>

#### Push

Добавляет элемент в стек. <br/> <br/>

Реализация: <br/>
Поскольку стек наполняется и опустошается сверху, то для пометки верхнего элемента в стеке необходимо создать ключ – top, он будет означать первый элемент в стеке. При добавлении нового элемента в стек нужно передвигать ключ top на новый элемент. <br/>
Также каждый элемент вставляемый в стек должен иметь как само значение, так и ссылку на нижний элемент. При добавлении нового элемента, автоматически присваивается ссылке нижний, предыдущий элемент. <br/> <br/>
...
Удаляет первый элемент из стека. <br/> <br/>

Реализация: <br/>
Поскольку стек наполняется и опустошается сверху, то для пометки верхнего элемент в стеке необходимо создать ключ – top, он будет означать первый элемент в стеке. <br/>
При вызове метода Remove необходимо переопределить ключ top, на следующий элемент в стеке, а верхний элемент отвязать от стека – присвоив ссылке в верхнем элементе - Null. <br/>
...
Проверяет пустой ли стек <br/> <br/>

Реализация: <br/>
К методам удаления и наполнения стека добавить инкремент и декремент. С исполнением методов push и remove увеличивается или уменьшается длина стека. Эта длина если больше 1, то метод возвращает false, если пустой – true. <br/>

Big O(1): <br/>
Возвращает или true или false в зависимости от длины стека, это 1 операция. <br/>

#### Peek

Получить первый элемент стека <br/>

Реализация: <br/>
Поскольку стек имеет ключ top, который всегда привязан к первому элементу в стеке. В результате суть метода заключается в том, чтобы возвращать значение, на которое ссылается top. <br/> <br/>
...
Особенность данной структуры является то, что элементы двигаются по системе FIFO – first input first out (первый зашел, первый вышел). <br/> Примерами из реальности служат, любые очереди в магазины, банки или подобное. <br/> Примерами же из сферы программирования есть возможность листать ленту в Instagram или TikTok. <br/>
...
Big O(1): <br/>
Поскольку каждый элемент имеет ссылку на предыдущий элемент, то у последнего элемента в очереди ссылка ссылается на Null. Внедрение записи о новом элементе в очереди это одна операция. <br/> <br/>
...
Реализация: <br/>
Поскольку очередь имеет ключ first, который всегда привязан к первому элементу в очереди. Суть метода заключается в том, чтобы возвращать значение, на которое ссылается first. <br/> <br/>
...
Реализация: <br/>
Поскольку очередь имеет ключ last, который всегда привязан к последнему элементу в очереди. Суть метода заключается в том, чтобы возвращать значение, на которое ссылается last. <br/> <br/>
...
Реализация: <br/>
При вставке любого из значений, это значение проходит условие на равенство с предыдущим значением, которое есть в дереве. Если значение больше, чем существующее - оно уходит в левую ветку, если меньше существующего – уходит в право. <br/> <br/>
Big O(log n): <br/>
...
Big O(log n): <br/>
При поиске элемента в дереве, элемент проходит условие на равенство с предыдущим значением, которое есть в дереве. Если значение больше, чем существующее - оно уходит в левую ветку, если меньше существующего – уходит в право. <br/>
...
Реализация: <br/>
При прохождении по дереву проверяется начальное значение, которое определяется уровнем расположения элемента в дереве с элементом. Этот элемент ответвлен с левой стороны плеча (поскольку по условию в левую сторону дерева отсылаются значения меньше номинального). <br/> <br/>
Big O((log n)/2): <br/>
При поиске минимального значения в дереве поиск всегда будет происходить по левому плечу, поскольку исходя из описанного метода структуры данных, в левое плечо уходит значение меньше от сравнимого. <br/> <br/>

## Хеш-таблица

Хеширование это метод для преобразований значений ключа в индекс. Зная этот индекс, можно получить очень быстро значения, при этом не важна сложность этих значений. <br/>
Хеш-таблица это ассоциативный массив, который состоит из индекса, преобразованного из ключа, с помощью хеширования и значения. Этот ключ, как раз и хранит этот индекс. <br/>
...
В рамках небольшого количества элементов матрица может быть и разумное решение, но если элементов миллион, к примеру в социальной сети есть миллион пользователей и одного из этих пользователей лишь один друг (связь с другим элементом), то в смежной матрице у одного элемента будет лишь одна истина (1), а остальные 999 999 элементов будут заполнены ложью (0), что делает такую матрицу огромной и при этом не рационально используемой.  <br/>
Структуру графа может быть представлена также смежным списком – объект, где каждый ключ - это название элемента, а значение ключа – массив, который состоит из связных элементов.  <br/>
...
Анаграммы - это разные слова, которые могут быть написаны с помощью одних и тех же букв с учётом их количества и длины самого слова.

Примеры: 
- `rail safety` и `fairy tales` - True
- `hi there` и `bye there` - False 

Примеры решения с кодом: [anagrams](algorithms/string-algorithms/anagrams.js)
